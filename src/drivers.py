...
class GameScraper(ChromeDriver):
    ...
    def get_teams_stats(self)-> None:
        ...
        """
        if self.link_tuples:    
            for link in self.link_tuples: # Link to teams pages are [0] and team name is [1]
                team = (link[1])                
                try: 
                    self.load_page(link[0]) # Load specific teams page
                    time.sleep(5)
                except Exception as e:
                    logger.error(f"Error in loading team page {e}")
                
                try:
                    game_by_game_link = self.driver.find_element(By.XPATH, "//a[contains(text(),'Game By Game')]")
                except NoSuchElementException as e:
                    # Since the game by game link is hidden in html find all links on the page 
                    game_by_game_link = self.driver.find_element(By.XPATH, "//a[contains(.,'Game By Game')]")
                
                self.load_page(game_by_game_link.get_attribute("href"))

                time.sleep(6)
                try:
                    self.get_game_by_game_data(team)
                except Exception as e:
                    logger.error(f"Error in getting game by game data for team {team}")
                    raise e
                
                # Need to bring driver back to the team links page in order to load the next team 
                self.driver.back()
                self.driver.back()

    def get_game_by_game_data(self, team: str)-> None:
        """
        ...
        for _ in range(max_retries):
            try:
                games_table = self.driver.find_element(By.XPATH,"//*[@id='game_breakdown_div']/table/tbody/tr/td/table")
                #games_table = self.driver.find_element(By.XPATH,"/html/body/div[2]/div[3]/table/tbody/tr/td/table/tbody")
                break
            except NoSuchElementException as e:
                logger.info(f"Error in finding games table {e}")
        ...
        try:
            self.stats = self.get_offensive_stats(games_table)
        except Exception as e:
            logger.error(f"Exception in getting offensive stats {e}")
            raise e
        
        try:
            self.get_defensive_stats()
        except Exception as e:
            logger.error(f"Exception in getting defensive stats {e}")
            raise e
        
        for row in self.stats: # Add team name and year data for each game 
            row.insert(1, team)
            row.append(self.year)
        
        self.scraped_data.append(self.stats)
 
    def get_offensive_stats(self, games_table:str)-> list:
        """
    ...
    def get_defensive_stats(self)-> None:
        """
        ...
        try:
            ...
            first_row_len = len(self.stats[0][1])
            ...
            logger.error(f"Error in retrieving defensive stats for {self.stats}")
...
def test_games():
    ...
    for year in years:
        ...
        scraper.find_team_links()
        scraper.get_teams_stats()
        for team in scraper.scraped_data:
    ...
    result.to_csv('game_data_2019_2020.csv')
    scraper.close()
