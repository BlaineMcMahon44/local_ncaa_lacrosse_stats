                ...
                <tbody>
                    {games.schedule.map(game => {
                        ...
                        if (daysDifference >= 0 && daysDifference <= 1) {
                            if (game["Home"] === 0){
                                return (
                                    <tr key={game["Team Name"]+game["Date"]}>
                                        <td>{stringDate}</td>
                                        <td><img src={teamLogos[game["Team Name"]]} alt={game["Team Name"]} style={{ height: '30px', width: '30px' }}></img> {game["Team Name"]}</td>
                                        <td><img src={teamLogos[game["Opponent"]]} alt={game["Opponent"]} style={{ height: '30px', width: '30px' }}></img> {game["Opponent"]}</td>
                                        <td>{game["Location"]}</td>
                                    </tr>
                                );
                            }
                        }
                        return null; // Don't render the row if it's not within 3 days
                    })}
                </tbody>
            </Table>
        </Container>
