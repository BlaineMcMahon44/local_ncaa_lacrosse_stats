...
import '../componentStyles/ScheduleTable.css';
...
function RecentGamesTable({ results }) {

    const currentDate = new Date();
    currentDate.setHours(0, 0, 0, 0); // Set hours to 00:00:00 for accurate comparison
                ...
                <tbody>
                    {results.length > 0 ? (
                        results.map((result, index) => {
                            const gameDate = formatDate(result.Date);
                            gameDate.setHours(0, 0, 0, 0); // Set hours to 00:00:00 for accurate comparison
                            const timeDifference = Math.abs(gameDate.getTime() - currentDate.getTime());
                            const daysDifference = Math.floor(timeDifference / (1000 * 3600 * 24)); // Convert milliseconds to days
                            const stringDate = dateToString(gameDate);
                                    ...
                                    <tr key={index}>
                                        ...
                                        <td>
                                            <div className="opponent-info">
                                                <span>{result.Team_Name.charAt(0).toUpperCase() + result.Team_Name.slice(1)}</span>
                                                <img src={teamLogos[result.Team_Name]} alt={result.Team_Name} className="team-logo" />
                                            </div>
                                        </td>
                                        <td>
                                            <div className="opponent-info">
                                                <span>{result.Opponent.charAt(0).toUpperCase() + result.Opponent.slice(1)}</span>
                                                <img src={teamLogos[result.Opponent]} alt={result.Opponent} className="team-logo" />
                                            </div>
                                        </td>
                                        <td style={{ color: result.L === 1 ? '#FF6347' : '#32CD32' }}>{result.Result}</td>
                                        <td>{result.OT}</td>
                                        <td>{result.Home}</td>
                                        <td>{result.Away}</td>
                                        <td>{result.Goals}</td>
                                        ...
                                        <td>{result.Saves}</td>
                                        <td>{result.FO_Won}</td>
                                        <td>{result.FOs_Taken}</td>
                                        <td>{result.Clear_Pct}</td>
                                        <td>{result.Man_Up_G}</td>
                                        <td>{result.Location}</td>
                                        <td>{result.Distance_Traveled}</td>
                                    </tr>
                                );
                            }
                            return null; // Don't render the row if it's not within 0 or 3 days
                        })
                    ) : (
                        <tr>
